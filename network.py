...
class GameServer:
    ...
    def get_init_packet(self):
        return {
            "enemies": {
                k: {"pos": v.get_normalised_pos(self.host_game.board), "type": v.size, "health": v.health}
                for k, v in self.host_game.enemies.items()
            },
            "players": {
                p.name: {
                    "pos": (p.x, p.y) if p != self.host_game.player_name else p.get_normalised_pos(self.host_game.board),
                    "num": self.players.index(p.name) + 1
                }
                for p in [self.host_game.player] + self.host_game.other_players
            },
            "bullets": {
                n: {
                    k: {"pos": (b.x, b.y), "colour": b.colour, "size": b.width, "from_enemy": b.from_enemy}
                    for k, b in v.items()
                }
    ...
    def get_synced_data(self, new_data):
        synced = {
            ...
            "players": {
                p.name: {
                    "pos": (p.x, p.y) if p != new_data["player"]["name"] else (new_data["player"]["pos"][0], new_data["player"]["pos"][1]),
                    "num": self.players.index(p.name) + 1
                }
                for p in [self.host_game.player] + self.host_game.other_players
            },
            "bullets": {
                n: {
                    k: {"pos": (b.x, b.y), "colour": b.colour, "size": b.width, "from_enemy": b.from_enemy}
                    for k, b in v.items() if k in new_data["bullets"][n]
                }
    ...
    def update_game_data(self, new_data):
        ...
        for k in new_data["players"]:
            if k in self.host_game.other_players:
                self.host_game.other_players[k].x = new_data["players"][k]["pos"][0]
                self.host_game.other_players[k].y = new_data["players"][k]["pos"][1]
            else:
                self.host_game.other_players[k] = Teammate(
                    new_data["players"][k]["pos"][0],
                    new_data["players"][k]["pos"][1],
                    k,
        ...
        for n in new_data["bullets"]:
            if n != self.host_game.player_name:
                for k in self.host_game.bullets[n]:
...
class GameClient:
    ...
    def load_json(self):
        return {
            ...
            "enemies": {
                k: {
                    "health": e.health,
                    "health_changed": self.previous_packet["enemies"][k]["health"] - e.health
            ...
            "bullets": {
                self.client_game.player_name: {
                    ...
                    } for k, b in self.client_game.bullets
                }
            }
        }
    ...
    def update_game_data(self, new_data):
        for k in new_data["enemies"]:
            if k in self.client_game.enemies:
                self.client_game.enemies[k].x = new_data["enemies"][k]["pos"][0]
                self.client_game.enemies[k].y = new_data["enemies"][k]["pos"][1]
                self.client_game.enemies[k].health = new_data["enemies"][k]["health"]
            else:
                self.client_game.enemies[k] = Enemy(
                    new_data["enemies"][k]["pos"][0],
                    new_data["enemies"][k]["pos"][1],
        ...
        for n in new_data["bullets"]:
            if n != self.client_game.player_name:
                for k in self.client_game.bullets[n]:
    ...
    def connect(self):
        ...
        self.init_packet = json.loads(self.sock.recv(2048).decode())
        ...
        self.client_game.player = Player(self.client_game.player_name, self.init_packet["players"][self.client_game.player_name]["num"])
